...
public class Consola {
	...
	public Usuario login() { //Pantalla de inicio para iniciar sesion
		...
		String cedula= ""+random.nextInt(59999999);
	...
	public boolean menu(Usuario comprador) {//Menu para reservar, consultar, modificar o eliminar la reserva
		...
		String opcion = ""+ (random.nextInt(5)+1);
		int numOpcion=1;
		if(esNumero(opcion))
	...
	public Dia seleccionarDia() {//Permite al usuario seleccionar un dia de la lista de dias
		...
		String numSelectDia = ""+(random.nextInt(SistemaDeReserva.getDias().size())+1);
		int selectDia=1;
		...
		if(selectDia>SistemaDeReserva.getDias().size()||selectDia<=0) {
			for(Dia dia:SistemaDeReserva.getDias())
				if(!dia.diaCompleto())
					return dia;
		}
